...
ICON_FILE = assets/icons/procview.ico
RC_FILE = $(RES_DIR)/icon.rc
RES_FILE = $(RES_DIR)/procview.res  # Store the compiled resource in assets/resources/

# Source files grouped into categories
SRC_FILES = $(wildcard $(SRC_DIR)/command/*.cpp) \
            $(wildcard $(SRC_DIR)/core/*.cpp) \
            $(wildcard $(SRC_DIR)/utils/*.cpp) \
            $(SRC_DIR)/procview.cpp  # Entry point

# Convert source files into object file paths
OBJ_FILES = $(patsubst $(SRC_DIR)/%.cpp, $(OBJ_DIR)/%.o, $(SRC_FILES))

# Output executable name
TARGET = $(BIN_DIR)/procview.exe  # .exe required on Windows
...
clean:
	...
	-if exist "$(BIN_DIR)\procview.exe" del /q "$(BIN_DIR)\procview.exe"
